...
class AtividadeController extends Controller
{
	...
	{
		$atividades = $this->atividade->orderBy('created_at', 'DESC')->get();
	...
	public function listarProgramaAtividade($id)
	{
		$usuario = Auth::user()->id;
		$permissoes = $this->permissoes_usuario->select(DB::raw('plano_id,programa_id'))->where('user_id',$usuario)->groupBy('plano_id','programa_id')->get();
		...
		$programas = $this->programa->where('plano_id',$id)->whereIn('id',$programas_id)->orderBy('created_at','desc')->get();
		return view('atividade.listar-programas-atividades',['programas' => $programas]);
	}

	public function listarAtividadesPorPrograma($id)
	{
		$atividades = $this->atividade->where('programa_id',$id)->orderBy('created_at', 'DESC')->get();
